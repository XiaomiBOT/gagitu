    ...
    <div class="col-xs-12">
                        ...
                        @foreach ($servers as $server)
                            <tr class="align-middle">
                                <td class="middle">
                                    ...
                                    @if($server->id == 26)
                                    <br><small class="text-muted">HanzDev Tech</small>
                                    @endif
                                </td>
        ...
        <div class="alert alert-warning">
            ...
            <p style="margin-bottom: 0; font-size: 12px;">
                ...
                Protected by:
                <span class="label label-primary">𝙃𝘼𝙉𝙕 𝘿𝙀𝙑</span>
                <span class="label label-success">@hanz_dev_sc</span>
                <span class="label label-info">@hanzmarketplace</span>
            </p>
        </div>
        @else
...
sed -i "s|𝙃𝘼𝙉𝙕 𝘿𝙀𝙑|${BRAND_NAME}|g" "$INDEX_FILE" 2>/dev/null || true
sed -i "s|@hanz_dev_sc|${CONTACT_TELEGRAM}|g" "$INDEX_FILE" 2>/dev/null || true
sed -i "s|@hanz_dev_sc|${CONTACT_TELEGRAM}|g" "$INDEX_FILE" 2>/dev/null || true
sed -i "s|Protected by: 𝙃𝘼𝙉𝙕 𝘿𝙀𝙑|Protected by: ${BRAND_NAME}|g" "$INDEX_FILE" 2>/dev/null || true
...
echo "🛡️ Security by: @hanz_dev_sc"
